...
test("vectorToCartesian0", () => {
    let out: ICoordinate = Transforms.VectorToCartesian(0, 10);
    expect(out.x).toBeCloseTo(0);
    expect(out.y).toBeCloseTo(-10);
});

test("vectorToCartesian360", () => {
    let out: ICoordinate = Transforms.VectorToCartesian(360, 10);
    expect(out.x).toBeCloseTo(0, 14);
    expect(out.y).toBeCloseTo(-10);
});

test("vectorToCartesian180", () => {
    let out: ICoordinate = Transforms.VectorToCartesian(180, 10);
    expect(out.x).toBeCloseTo(0, 14);
    expect(out.y).toBeCloseTo(10);
});

test("vectorToCartesian90", () => {
    let result: ICoordinate = Transforms.VectorToCartesian(90, 10);
    expect(result.x).toBeCloseTo(10);
    expect(result.y).toBeCloseTo(0, 14);
});

test("vectorToCartesian270", () => {
    let result: ICoordinate = Transforms.VectorToCartesian(270, 10);
    expect(result.x).toBeCloseTo(-10);
    expect(result.y).toBeCloseTo(0, 14);
});

test.each([[30, 10, 5, -8.66]])("VectorToCartesians", (angle, length, expX, expY) => {
    let result: ICoordinate = Transforms.VectorToCartesian(angle, length);
...
test.each([[4, 3, 53.13, 5]])("CartesianToVector", (x, y, expAngle, expLength) => {
    let result: IVector = Transforms.CartesianToVector(x, y);
...
test.each([[2,2,2,4,4]])("Scale", (x, y, scale, expX, expY) => {
    let result: ICoordinate[] = Transforms.Scale([{x: x, y:y}], scale, scale);
