...
export class ControlPanelView implements IView {
    ...
    display(drawContext: DrawContext) {
        ...
        // draw selector
        drawContext.drawRect(this.location.x, this.location.y + this.model.selectedControl * 20, 5, 5);
    }
}
...
export class SliderView implements IView {
    ...
    display(drawContext: DrawContext) {
        ...
        if (this.model.enabled) {
            drawContext.drawRect(this.location.x + 10, this.location.y, 2, 2);
        }
        drawContext.drawText(this.location.x+20, this.location.y+this.fontSize, this.model.name, this.fontSize, this.font);
        
        drawContext.drawRect(this.location.x + 140, this.location.y, this.width * factor, 10);
